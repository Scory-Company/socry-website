...
import { Search, Filter, Edit, CheckCircle, Layers, Clock, LayoutGrid, List as ListIcon, ChevronDown, FileText, BookOpen } from "lucide-react"
...
export default function AuthorArticlesPage() {
  ...
  const fetchArticles = useCallback(async (page: number = 1) => {
    setIsLoading(true)
    try {
      // NOTE: The backend API handles basic filtering. 
      // We keep the logic aligned with previous implementation but cleaner.
      ...
      // Logic Preservation: Client-side sorting/filtering enhancements
      let processedData = [...data]
      
      // Additional client-side filtering if needed (keeping legacy logic just in case backend behaves strictly)
      if (debouncedSearch.trim()) {
         const lowerQ = debouncedSearch.toLowerCase().trim()
         // Only filter if backend didn't do it (safety net)
  ...
  return (
    <div className="min-h-screen bg-background p-6 space-y-6">
      ...
      {/* Toolbar */}
      <div className="bg-card border border-border rounded-xl p-4 shadow-sm flex flex-col xl:flex-row xl:items-center gap-4 sticky top-4 z-20 backdrop-blur-md bg-card/95 supports-[backdrop-filter]:bg-card/80">
                        ...
                        {viewMode === 'grid' ? (
                            ...
                            <div className="group h-full bg-card border border-border rounded-xl overflow-hidden hover:border-primary/50 hover:shadow-lg transition-all flex flex-col relative w-full">
                                ...
                                <div className="p-3 bg-muted/40 border-t border-border">
                                    <Link href={`/author/articles/${article.id}/edit`} className="flex items-center justify-center gap-2 w-full py-2 bg-background border border-border rounded-lg text-sm font-semibold hover:bg-primary hover:text-primary-foreground transition-all">
